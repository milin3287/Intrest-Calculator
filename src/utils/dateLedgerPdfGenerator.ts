...
export function downloadDateLedgerPDF(options: DateLedgerPDFOptions): void {
  ...
  doc.text(
    `• Generated by MRP Interestly Financial Suite. Certified mathematical output. Verified safe for personal, peer-to-peer, and commercial bookkeeping.`,
    margin + 3,
    summaryBoxY + 14.5
  );
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text('MRP Interestly — Precision Date-to-Date Financial Engine', margin, pageHeight - 5.5);
    doc.text(
...
export function downloadMonthlyBucketPDF(
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(`MRP Interestly — Monthly Statement (${bucket.monthLabel})`, margin, pageHeight - 5.5);
    doc.text(
      `Page ${i} of ${totalPages}`,
      pageWidth - margin - doc.getTextWidth(`Page ${i} of ${totalPages}`),
      pageHeight - 5.5
